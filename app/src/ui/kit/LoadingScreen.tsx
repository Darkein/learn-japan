...
interface Props {
  /** Libellé sous l'ensō. Par défaut « 日本語 », comme le titre du header. */
  label?: string;
  /** Classes en plus (rare — ajuster la hauteur mini par ex.). */
  className?: string;
}

export function LoadingScreen({ label = "日本語", className = "" }: Props) {
    ...
    >
      {painting && (
        <>
          ...
          {/* Même signature que le titre du header : sérif, accent (cf. App.tsx). */}
          <p className="enso-appear font-serif text-lg tracking-wide text-accent">{label}</p>
        </>
      )}
    </div>
  );

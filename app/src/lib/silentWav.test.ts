...
describe("buildSilentWavBlob", () => {
  it("produit un blob audio/wav de la bonne taille (44 octets d'en-tête + 4000 éch./s)", () => {
    const b = buildSilentWavBlob(8000);
    expect(b.type).toBe("audio/wav");
    expect(b.size).toBe(44 + 4000 * 8);
  });

  it("arrondit les durées non entières en échantillons (ex. 250 ms → 1000 échantillons)", () => {
    expect(buildSilentWavBlob(250).size).toBe(44 + 1000);
  });

  it("écrit un en-tête RIFF/WAVE cohérent et un payload 100 % silence", async () => {
    ...
    expect(view.getUint32(4, true)).toBe(36 + 4000); // taille RIFF
    expect(view.getUint32(24, true)).toBe(4000); // sample rate
    expect(view.getUint32(40, true)).toBe(4000); // taille data

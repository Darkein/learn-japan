...
describe("buildSilentWavBlob", () => {
  it("produit un blob audio/wav de la bonne taille (44 octets d'en-tête + 24 000 éch./s × 2 octets)", () => {
    const b = buildSilentWavBlob(8000);
    expect(b.type).toBe("audio/wav");
    expect(b.size).toBe(44 + 24000 * 8 * 2);
  });

  it("arrondit les durées non entières en échantillons (ex. 250 ms → 6000 échantillons)", () => {
    expect(buildSilentWavBlob(250).size).toBe(44 + 6000 * 2);
  });

  it("écrit un en-tête RIFF/WAVE 24 kHz (fréquence des MP3 Cloud TTS) au payload quasi-silencieux mais PAS nul", async () => {
    ...
    expect(view.getUint32(4, true)).toBe(36 + 24000 * 2); // taille RIFF
    expect(view.getUint32(24, true)).toBe(24000); // sample rate
    expect(view.getUint16(34, true)).toBe(16); // bits par échantillon
    expect(view.getUint32(40, true)).toBe(24000 * 2); // taille data
